...
customElements.define('ddhi-viewer', class extends DDHIDataComponent {
  constructor() {
    ...
    this.teiResource; // The associated TEI resource
    this.teiLink; // URL of the TEI file
    this.media = []; // The associated media object, if any
    this.primaryRecordingUrl; // URL of the primary recording
  ...
  async attributeChangedCallback(attrName, oldVal, newVal) {
    if(attrName == 'ddhi-active-id') {
      ...
      // get Primary Audio from subresource
      
      await this.getMedia(this.getAttribute('ddhi-active-id'));
      
      this.primaryRecordingUrl = '';
      
      if (typeof this.media !== 'unknown') {
        for(var i=0;i < this.media.length;i++) {
          if (this.media[i].id == 'primary_recording') {
            this.primaryRecordingUrl = this.media[i].url;
          }
        }
      }
      
      var audioPlayerElement = this.shadowRoot.getElementById('media-player').querySelector('audio');
      audioPlayerElement.setAttribute('src',this.primaryRecordingUrl);
      
      if (this.primaryRecordingUrl == '') {
        audioPlayerElement.classList.add('disabled');
      } else {
        audioPlayerElement.classList.remove('disabled');
      }
  ...
  async getMedia(id,format='json') {
          
      const response = await fetch(this.apiURI + '/items/' + id + '/media?_format=' + format, {mode: 'cors'});
      const result = await response.json();
...
customElements.define('ddhi-entity-map', class extends DDHIVisualization {
  constructor() {
    ...
    leafletExport.setAttribute('src','//api.tiles.mapbox.com/mapbox.js/plugins/leaflet-image/v0.0.4/leaflet-image.js');
...
customElements.define('ddhi-timeline', class extends DDHIVisualization {
      ...
      <div class='controls'>
          ...
          <div class="range-controls">
            ...
            <div class="y-controls">
              ...
              <span class="button-desc">Interview</span>
  ...
  connectedCallback() {
    ...
    async getItemDataById() {
      ...
      if (activeId !== null) {
        component.tempResult = null;
        var response = await this.getAssociatedEntitiesByType(this,'',activeId,'transcripts'); 
        this.itemsDataSetItem(activeId,component.tempResult);
        component.tempResult = null;
      }   
    ...
    changeViewer(timelineType) {      
      const currTimeline = this.shadowRoot.querySelector('#container');
      ...
      if(timelineType === 'all') {
        currTimeline.style.display = 'block';
